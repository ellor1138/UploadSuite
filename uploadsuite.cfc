	...
	component output=false {
		...
		*/
		public function init() {
			...
			if ( !isDefined("application.wheels.UploadSuiteMimeTypesList") ) {
				loc.lhs = CreateObject("java", "java.util.LinkedHashSet");

				application.wheels.UploadSuiteMimeTypesList = "";

				// Create list from wheels mime type
				for ( loc.key IN application.wheels.mimetypes ) {
					application.wheels.UploadSuiteMimeTypesList = ListAppend(application.wheels.UploadSuiteMimeTypesList, application.wheels.mimetypes[loc.key]);
				}

				// Create list from wheels format
				for ( loc.key IN application.wheels.formats ) {
					application.wheels.UploadSuiteMimeTypesList = ListAppend(application.wheels.UploadSuiteMimeTypesList, application.wheels.formats[loc.key]);
				}

				// Remove duplicate
				application.wheels.UploadSuiteMimeTypesList = ArrayToList(loc.lhs.init(ListToArray(application.wheels.UploadSuiteMimeTypesList)).toArray());
			}

			// Remove banned mime type
			if ( isDefined("application.wheels.uploadSuiteBannedMimeTypes") ) {
				for ( loc.key = 1; loc.key <= ListLen(application.wheels.uploadSuiteBannedMimeTypes); loc.key++ ) {
					if ( ListFind(application.wheels.UploadSuiteMimeTypesList, ListGetAt(application.wheels.uploadSuiteBannedMimeTypes, loc.key)) ) {
						application.wheels.UploadSuiteMimeTypesList = ListDeleteAt(application.wheels.UploadSuiteMimeTypesList, ListFind(application.wheels.UploadSuiteMimeTypesList, ListGetAt(application.wheels.uploadSuiteBannedMimeTypes, loc.key)));
					}
				}
			}
		...
		*/
		public function ajaxFileUploadTag(
			...
			string mimeTypesList="#application.wheels.UploadSuiteMimeTypesList#",
			string localeid="",
			...
			if ( !arguments.minimalist ) {
				...
				loc.js = "
					<script type='text/javascript'>
						...
						function setMessage#arguments.uuid#(uuid, messageType) {
							// console.log(uuid);
									...
									if ( xhr#arguments.uuid#.status == 200 ) {
										try {
											...
											if ( responseJSON.FILEWASSAVED ) {
												setMessage#arguments.uuid#(uuid, 'success');
